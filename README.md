...
需要让本机 AI 客户端查询寄存器库时，可以单独构建只读 stdio MCP companion binary。它不启动网络监听，不提供导入或修改工具，默认也不会返回用户备注正文。构建、`--db`、客户端配置和隐私边界详见 [docs/mcp.md](docs/mcp.md)。当前 MCP binary 尚未放入便携包，仍作为独立产物分发。
